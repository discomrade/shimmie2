...
class PermManagerTheme extends Themelet
{
    ...
    public function display_table(Page $page, bool $is_parent, array $parent_options, array $permissions, UserClass $class): void
    {
        ...
        $perm_form = SHM_SIMPLE_FORM(
            ...
            $is_parent ? B("WARNING: This class is a parent to another class. Modifying this class will do the same for any child classes.") : emptyHTML(),
            TABLE(
                ...
                $tbody,
                $editable ? TFOOT(TR(TD(["colspan" => '5'], INPUT(["type" => 'submit', "value" => 'Set Permissions'])))) : emptyHTML()
            )
        );
        ...
        $page->add_block(new Block($title, emptyHTML()));
        if ($editable) {
